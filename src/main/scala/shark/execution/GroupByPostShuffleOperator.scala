...
import org.apache.hadoop.hive.ql.abm.udaf.{GenericUDAFEvaluatorWithInstruction, CondMergeEvaluator}
...
class GroupByPostShuffleOperator extends GroupByPreShuffleOperator 
  ...
  override def createLocals() {
    ...
    if (conf.isUncertain) {
      val totalTupleNumber = conf.getTotalTupleNumber
      aggregationEvals.foreach(_.asInstanceOf[GenericUDAFEvaluatorWithInstruction].setTotalTupleNumber(totalTupleNumber))
      aggregationEvals.last.asInstanceOf[CondMergeEvaluator].setFlags(conf.getFlags)
    }
  }
  ...
  override def execute(): RDD[_] = {
    ...
    // ABM: we only modify hashAggregate because we should never use sortAggregate
            ...
            if (distinctKeyAgg != null) {
              ...
              while (distinctKeyAggIter.hasNext) {
                ...
                if (isNewDistinct) {
                  ...
                  var pi = 0
                  while (pi < o.length) {
                    lastDistinctKey(pi) = ObjectInspectorUtils.copyToStandardObjectForShark(
  ...
  def hashAggregate(iter: Iterator[_]) = {
    ...
    val newIter = iter.map { case (key: ReduceKeyReduceSide, values: Seq[Array[Byte]]) =>
      ...
      i = aggrs.length - 1
      while (i >= 0) {
        outputCache(i + numKeys) = aggregationEvals(i).evaluate(aggrs(i))
        i -= 1
      }
      outputCache
    }
