...
class SharkSemanticAnalyzer(conf: HiveConf) extends SemanticAnalyzer(conf) with LogHelper {
  ...
  override def analyzeInternal(ast: ASTNode): Unit = {
    ...
    AbmUtilities.setSpecialQuery(astTokenType == HiveParser.TOK_CREATEVIEW
      || astTokenType == HiveParser.TOK_ANALYZE
      || astTokenType == HiveParser.TOK_CREATETABLE)
    ...
    if (AbmUtilities.inAbmMode()) {
      AbmUtilities.recordViewSchema(pctx.getOpParseCtx.get(hiveSinkOp).getRowResolver)
    } else {
     ...
     * print before plan
     */
    // AbmTestHelper.printBeforeRewritePlan(hiveSinkOp);
    ...
    if (AbmUtilities.inAbmMode()) {
      _resSchema = AbmUtilities.convertRowSchemaToViewSchema(pctx.getOpParseCtx.get(hiveSinkOp).getRowResolver)
      val loadWorks = getParseContext.getLoadFileWork
      assert (loadWorks.size() == 1)
      val loadWork = loadWorks.get(0)
      loadWork.setColumns(_resSchema.map(_.getName).mkString(","))
      loadWork.setColumnTypes(_resSchema.map {
        field =>
          val t = field.getType
          if (t.equals(serdeConstants.VOID_TYPE_NAME)) serdeConstants.STRING_TYPE_NAME else t
      }.mkString(":"))
    }
     ...
     * print after plan
     */
    AbmTestHelper.printAfterRewritePlan(hiveSinkOp, pctx);
